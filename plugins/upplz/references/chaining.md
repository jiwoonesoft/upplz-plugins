...
- **upplz-onboard** 완료 → 제안: 스크린샷 생성?(upplz-screenshot) / 메타데이터 정리?(upplz-metadata) / 테스터 추가?(add_testers)
- **upplz-build** 완료 → 제안: 아이콘 갱신?(upplz-icon) / 스크린샷 갱신?(upplz-screenshot) / 메타데이터 갱신?(upplz-metadata) / 테스터 추가?
