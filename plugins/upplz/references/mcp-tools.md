...
## 빌드/업로드
- `start_build` — 로컬 빌드 스크립트 + 임시 api_key(.p8) + UPPLZ_* env 반환. `.upplz/App.ipa` 생성. 사전조건 미충족 시 PRECONDITION_FAILED / PROVISIONING_PROFILE_MISSING / MARKETING_VERSION_REQUIRED 게이트. repoUrl 필수. projectType 미지정 시 직전 빌드 값→web-capacitor 기본(native 첫 빌드는 반드시 명시, 모호 시 scheme/xcodeProjectPath). 스크립트 실행 시 MATCH_PASSWORD env 필요.
