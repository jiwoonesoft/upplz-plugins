...
고객에게 `upplz` 플러그인을 배포하는 방법. 설치 한 번으로 **스킬 5개 + 원격 MCP 서버 연결 + API 키 구성**이 함께 완료된다.
...
- **API 키 발급**: 고객은 upplz 대시보드 설정 페이지(`create_upload_link`)에서 본인 키를 먼저 발급받아야 한다.
...
- 고객은 설치 시 `userConfig` 프롬프트에서, upplz 온보딩(대시보드 설정 페이지)으로 안내받은 **실제 엔드포인트**를 입력한다.
