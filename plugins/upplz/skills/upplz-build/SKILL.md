...
- Apple 처리(5–30분) 후 TestFlight에서 새 빌드 확인 가능함을 안내.

## Next
`${CLAUDE_PLUGIN_ROOT}/references/chaining.md`에 따라 물어본다: 아이콘 갱신?(upplz-icon) / 스크린샷 갱신?(upplz-screenshot) / 메타데이터 갱신?(upplz-metadata) / 테스터 추가?(add_testers — report_build_result로 uploaded 보고가 끝난 빌드만).
- 새 버전을 스토어에 출시하려면 심사 제출은 App Store Connect 웹에서 수동(Add for Review → Submit for Review)임을 안내한다.
